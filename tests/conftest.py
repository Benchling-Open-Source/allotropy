...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    ...
    if "test_file_path" in metafunc.fixturenames and metafunc.cls.VENDOR:
        ...
        if metafunc.cls.INCLUDE_FILTER:
            paths = [path for path in paths if any(re.search(regex, str(path)) for regex in metafunc.cls.INCLUDE_FILTER)]
        if metafunc.cls.EXCLUDE_FILTER:
            paths = [path for path in paths if not any(re.search(regex, str(path)) for regex in metafunc.cls.EXCLUDE_FILTER)]
