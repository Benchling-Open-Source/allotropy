...
def optimize_quantstudio_txt(filepath: str) -> None:
    ...
    print(
        f"Optimized: {len(output)} lines ({100*(1-len(output)/len(lines)):.1f}% reduction)"
    )
...
def optimize_biorad_xml(filepath: str) -> None:
    ...
    print(
        f"Optimized: {len(output)} lines, kept {min(well_count, 8)}/{well_count} wells"
    )
...
for filepath, optimizer in files:
    ...
    print(
        f"Size: {size_before:.1f}MB → {size_after:.1f}MB ({100*(1-size_after/size_before):.1f}% reduction)"
    )
