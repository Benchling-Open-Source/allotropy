...
class SeriesData:
    ...
    def _sanitize_dict_keys(self, data: dict[str, float | str | None]) -> dict[str, float | str | None]:
    ...
    def _get_matching_keys(self, key_or_keys: str | set[str]) -> set[str]:
        return {
            ...
            for matched in [k for k in self.series.index if k == regex_key or re.fullmatch(regex_key, k)]
        }
    ...
    def get_custom_keys(
        ...
        return {
            key.lstrip(): self._get_custom_key(key)
    ...
    def get_unread(
        ...
        return self._sanitize_dict_keys(self.get_custom_keys(matching_keys - self.read_keys))
