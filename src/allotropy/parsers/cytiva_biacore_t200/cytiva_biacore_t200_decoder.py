...
from allotropy.parsers.cytiva_biacore_t200.cytiva_biacore_t200_utils import (
    write_parquet,
)
...
def get_sample_data(sample_json: dict[str, Any]) -> list[dict[Any, Any]]:
    sample_data = []
    sample_details = []
    positions = sample_json["positions"]
    position_order = sample_json["position_order"]
...
def process_and_rearrange_xml_data(data: dict[Any, Any]) -> dict[Any, Any]:
...
def get_detection_data(
    detection_settings: dict[str, list[dict[Any, Any]]]
) -> dict[Any, Any]:
    detection_data = {}
...
def get_data_item_data(data_item: dict[Any, Any]) -> dict[Any, Any]:
    data_time = {}
...
def decode_application_data(
    application_template_data: dict[Any, Any]
) -> tuple[dict[str, dict[Any, Any]], list[dict[Any, Any]]]:
    """
    ...
    """
    total_samples: list[dict[Any, Any]] = []
                ...
                if xml_name["@name"] == "_Racks":
                    ...
                    for xml_string in xml_name["string"]:
                        ...
                        if "methodSettings" in xml_string["@value"]["method"]:
                            ...
                            measurement_settings = get_data_item_data(
                                xml_string["@value"]["method"]["methodSettings"][
                                    "dataItems"
                                ]
                            )
...
def extract_stream_data(data: str) -> dict[Any, Any]:
    """
...
def decode_data(input_data: IOType, file_path: str) -> dict[Any, Any]:
    ...
    for stream in streams:
        ...
        if stream == ["Environment"]:
            ...
            for _name, group in report_point_data:
                ...
                write_parquet(rpoint_parquet_path, group)
                ...
                segment_data_frame = pd.DataFrame(
                    {
                        ...
                        "Window Number": [cycle["window_number"]]
                        * length_of_segment,
    ...
    for _name, group in sensorgram_data:
        ...
        write_parquet(sensorgram_parquet_path, group)
