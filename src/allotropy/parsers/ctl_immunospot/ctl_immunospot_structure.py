...
def _create_measurement(
    ...
    data_processing_document = {
        "Min. SpotSize": (
            TQuantityValue(value=v, unit="mm^2")
            if (v := get_value_or_none(header_data, "Min. SpotSize")) is not None
            else None
        ),
        "Max. SpotSize": (
            TQuantityValue(value=v, unit="mm^2")
            if (v := get_value_or_none(header_data, "Max. SpotSize")) is not None
            else None
        ),
