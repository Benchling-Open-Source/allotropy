...
def convert_datetime(x: pd.Series[str]) -> pd.Series[str]:
    return pd.to_datetime(x).dt.strftime("%Y-%m-%d %H:%M:%S")


def convert_float(x: pd.Series[str]) -> pd.Series[str]:
    return pd.to_numeric(x, errors="coerce")


def convert_int(x: pd.Series[str]) -> pd.Series[str]:
    return pd.to_numeric(x, errors="coerce")


def convert_string(x: pd.Series[str]) -> pd.Series[str]:
    return x.astype(str)
