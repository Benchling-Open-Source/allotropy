...
class PlateInformation:
    ...
    def create(contents: MabTechApexContents) -> PlateInformation:
        ...
        return PlateInformation(
            ...
            software_version=try_str_from_series_or_none(plateinformation, key="Software Version:"),
...
class Well:
    ...
    @staticmethod
    def create(plate_data: pd.Series) -> Well | None:
        ...
        for feature in IMAGE_FEATURES:
            image_dict = {str(feature): try_float_from_series_or_none(plate_data, feature)}
            image_feature_list.append(image_dict)

        return Well(
            measurement_time=try_str_from_series_or_none(plate_data, "Read Date"),
            ...
            illumination_setting=try_float_from_series_or_none(plate_data, "Preset Intensity"),
