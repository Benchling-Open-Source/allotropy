...
class MabTechApexParser(VendorParser):
    ...
    def _get_model(self, data: PlateInformation, wells: WellList, file_name: str) -> Model:
        return Model(
            plate_reader_aggregate_document=PlateReaderAggregateDocument(
                ...
                plate_reader_document=[
                    PlateReaderDocumentItem(
                        ...
                        measurement_aggregate_document=self.get_measurement_aggregate_document(well),
                    )
                    for well in wells
                ]
            ),
    ...
    def get_measurement_aggregate_document(self, well: Well) -> MeasurementAggregateDocument:
        return MeasurementAggregateDocument(
            ...
            measurement_document=[
                self.get_measurement_document_item(well)
            ],
        )

    def get_measurement_document_item(
            self, well: Well) -> OpticalImagingMeasurementDocumentItems:
    ...
    def get_device_control_document_item(self, well: Well) -> OpticalImagingDeviceControlDocumentItem:
            ...
            image_feature_aggregate_document=ImageFeatureAggregateDocument(
                image_feature_document=[
                    ImageFeatureDocumentItem(
                        ...
                        image_feature_result=TQuantityValueUnitless(value=image_feature.get(next(iter(image_feature.keys()))))
                    )
