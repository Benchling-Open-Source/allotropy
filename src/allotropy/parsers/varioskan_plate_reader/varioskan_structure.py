...
class VarioskanMetadata:
    @staticmethod
    def create_metadata(instrument_info_df: pd.DataFrame, general_info_df: pd.DataFrame) -> Metadata:
        software_info = None
        ...
        return Metadata(
            device_identifier=VarioskanMetadata.find_value_by_label(instrument_info_df, "Name"),
            model_number=VarioskanMetadata.find_value_by_label(instrument_info_df, "Name"),
            software_name=software_name,
            software_version=version_number,
            unc_path="",
            file_name="",
            equipment_serial_number=VarioskanMetadata.find_value_by_label(instrument_info_df, "Serial number"),
...
class AbsorbanceDataWell(Measurement):
    @staticmethod
    def create(well_location, abs_value, sample_name, detector_wavelength) -> Measurement:
        return Measurement(
            type_="Absorbance",
...
class VarioskanMeasurementGroup(MeasurementGroup):
    ...
    def create(absorbance_sheet_df, layout_definitions_df, session_info_df):
        abs_df, name_df, wavelength = VarioskanMeasurementGroup.identify_abs_and_sample_dfs(absorbance_sheet_df)
        plate_well_count = VarioskanMeasurementGroup.get_plate_well_count(layout_definitions_df)
        session_name = VarioskanMeasurementGroup.find_value_by_label(session_info_df, "Session notes")
        exec_time = VarioskanMeasurementGroup.find_value_by_label(session_info_df, "Execution Time")
        absorbance_wells = []
    ...
    def get_plate_well_count(layout_definitions_df):
        ...
        plate_template_row = layout_definitions_df[
            layout_definitions_df.iloc[:, 0].str.contains('Plate template', case=False, na=False)]
        plate_template_value = plate_template_row.iloc[0].dropna().str.cat(
            sep=' ') if not plate_template_row.empty else None
        # Extract the integer from the combined string
        plate_template_number = int(re.search(r'\d+', plate_template_value).group() if plate_template_value else None)
    ...
    def identify_abs_and_sample_dfs(absorbance_sheet_df):
        ...
        for index, row in absorbance_sheet_df.iterrows():
            if pd.notna(row[0]) and "Wavelength" in row[0]:
                wavelength = int(re.search(r'Wavelength:\s*(\d{3})\s*nm', row[0]).group(1))
        ...
        df_between_abs_and_blank = pd.DataFrame(data_between_abs_and_blank).reset_index(drop=True)
        df_between_sample_and_blank = pd.DataFrame(data_between_sample_and_blank).reset_index(drop=True)
...
class DataVarioskan(Data):
    ...
    def create(sheet_data: dict[str, pd.DataFrame], file_name:str) -> Data:
        ...
        metadata = VarioskanMetadata.create_metadata(instrument_info_df=inst_info_df,
                                                     general_info_df=general_df)
        measurement_group = VarioskanMeasurementGroup.create(absorbance_sheet_df=abs_df,
                                                             layout_definitions_df=layout_df, session_info_df=session_df)
