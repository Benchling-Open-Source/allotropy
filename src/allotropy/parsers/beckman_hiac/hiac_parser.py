...
class HIACParser(VendorParser):
    ...
    def _extract_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        data.columns = [x.strip() for x in data.loc[0]]
        data = data.loc[1:, :]
        ...
        for elem in df.to_dict("records"):
            ...
            for c in cols:
                ...
                if c in elem:
                    ...
                    item[prop] = None
            d.items.append(DistributionItem(**item))
    ...
    def _setup_model(self, df: pd.DataFrame) -> Model:
        ...
        props = {
            "dilution_factor_setting": df.loc[13, 2],
            "detector_model_number": df.loc[2, 5],
            "analyst": df.loc[6, 5],
            "repetition_setting": df.loc[11, 5],
            "sample_volume_setting": df.loc[11, 2],
            "detector_view_volume": df.loc[9, 5],
            "measurement_identifier": df.loc[2, 2],
            "sample_identifier": df.loc[2, 2],
            "equipment_serial_number": str(df.loc[4, 5]),
            "detector_identifier": str(df.loc[4, 5]),
            "measurement_document": MeasurementDocument(
                distribution_document=distribution_document
            ),
            "measurement_time": (
                pd.to_datetime(df.loc[8, 5].replace(".", "-")).isoformat(
