...
def get_str_from_series(series: pd.Series, key: str, default: Optional[str] = None) -> Optional[str]:
...
def map_well_data(data: pd.DataFrame, data_fn: Callable[[pd.DataFrame], T]) -> dict[int, T]:
    return {
        assert_not_none(try_int(well_id)): data_fn(well_data)
...
class Header:
    ...
    def create(reader: LinesReader) -> Header:
        ...
        regex_result = assert_not_none(re.search(time_regex, raw_time), f"Unable to understand time format {raw_time}").group()
        measurement_time = assert_not_none(get_timestamp(regex_result, "%Y-%m-%d %H:%M:%S %p")).strftime("%Y-%m-%d %H:%M:%S")

        block_type = assert_str_from_series(data, "Block Type")
        number_match = assert_not_none(re.match("[1-9][0-9]+", block_type), msg="Block type missing number prefix")
        ...
        experiment_type = assert_not_none(experiments_type_options.get(assert_str_from_series(data, "Experiment Type"), None), msg="Unable to get valid experiment type")
        ...
        return Header(
            ...
            device_serial_number=assert_str_from_series(data, "Instrument Serial Number"),
            measurement_method_identifier=assert_str_from_series(data, "Quantification Cycle Method"),
            qpcr_detection_chemistry=assert_str_from_series(data, "Chemistry"),
            passive_reference_dye_setting=get_str_from_series(data, "Passive Reference"),
...
class WellItem:
    ...
    @staticmethod
    def create(data: pd.Series, target_dna_description: Optional[str] = None) -> WellItem:
        return WellItem(
            identifier=assert_not_none(try_int(get_str_from_series(data, "Well")), "Well item id"),
            target_dna_description=target_dna_description or assert_str_from_series(data, "Target Name"),
...
class Well:
    ...
    def create(data: pd.DataFrame) -> list[Well]:
        return [
            Well(
                identifier=assert_not_none(try_int(well_id), "Well id"),
...
def create_wells(reader: LinesReader, experiment_type: ExperimentType) -> list[Well]:
    raw_data = reader.pop_csv_block(r"^\[Sample Setup\]", sep="\t").replace(np.nan, None)
...
class AmplificationData:
    ...
    def create(reader: LinesReader) -> dict[int, dict[str, AmplificationData]]:
        ...
        return map_well_data(data, lambda well_data: {
            target_name: AmplificationData(
...
class MulticomponentData:
    ...
    def create(reader: LinesReader) -> Optional[dict[int, MulticomponentData]]:
        ...
        return map_well_data(data, lambda well_data: MulticomponentData(
    ...
    def get_column(self, name: str) -> list[Optional[float]]:
        return assert_not_none(self.columns.get(name), f"column {name} in multicomponent data")
...
class Result:
    ...
    def create(series: pd.Series, allele: Optional[str] = None) -> Result:
        ...
        return Result(
            cycle_threshold_value_setting=assert_not_none(try_float(get_str_from_series(series, f"{prefix}Ct Threshold")), "Ct Threshold"),
            cycle_threshold_result=try_float(get_str_from_series(series, f"{prefix}Ct" if prefix else "CT")),
            automatic_cycle_threshold_enabled_setting=bool_or_none(series, f"{prefix}Automatic Ct Threshold"),
            automatic_baseline_determination_enabled_setting=bool_or_none(series, f"{prefix}Automatic Baseline"),
            normalized_reporter_result=try_float(get_str_from_series(series, "Rn")),
            baseline_corrected_reporter_result=try_float(get_str_from_series(series, f"{prefix}Delta Rn")),
            genotyping_determination_result=get_str_from_series(series, "Call"),
            genotyping_determination_method_setting=try_float(get_str_from_series(series, "Threshold Value")),
        )


def get_results(reader: LinesReader, experiment_type: ExperimentType) -> dict[int, dict[str, Result]]:
    data = reader.pop_csv_block(r"^\[Results\]", sep="\t")

    if experiment_type == ExperimentType.genotyping_qPCR_experiment:
        return map_well_data(data, lambda well_data: {
            f"{target_name}-{allele}": Result.create(df_to_series(target_data), allele)
            for target_name, target_data in well_data.groupby("SNP Assay Name")
            for allele in set(col[:-13] for col in target_data.columns if re.match(r"\w+ Ct Threshold", col))
        })
    else:
        return map_well_data(data, lambda well_data: {
            target_name: Result.create(df_to_series(target_data))
            for target_name, target_data in well_data.groupby("Target Name")
        })
...
class MeltCurveRawData:
    ...
    def create(reader: LinesReader) -> Optional[dict[int, MeltCurveRawData]]:
        ...
        return map_well_data(data, lambda well_data: MeltCurveRawData(
    ...
    def create(reader: LinesReader) -> Data:
        ...
        for well in wells:
            ...
            for well_item in well.items:
                well_item.amplification_data = amp_data[well_item.identifier][well_item.target_dna_description]
                well_item.result = results_data[well_item.identifier][well_item.target_dna_description.replace(" ", "")]
