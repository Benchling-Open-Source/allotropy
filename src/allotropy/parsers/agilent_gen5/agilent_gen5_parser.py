...
def merge_measurement_groups(
    ...
    # Merge measurements for each well in sorted order for consistency
    merged_groups = []
    for well_pos in sorted(groups_by_well.keys()):
    ...
    def create_data(self, named_file_contents: NamedFileContents) -> Data:
        ...
        for read_index, read_data in enumerate(context.read_data, start=1):
            ...
            if is_kinetic and len(read_data.measurement_labels) > 1:
                ...
                sorted_labels = sorted(read_data.measurement_labels, key=lambda x: (len(x), x))
                ...
                # Process the read as a whole (either endpoint, or kinetic with single label)
