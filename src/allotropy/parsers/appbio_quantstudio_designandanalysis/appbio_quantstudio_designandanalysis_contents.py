...
class DesignQuantstudioContents:
    def __init__(self, contents: IOType) -> None:
        ...
        self.contents = {name: df.replace(np.nan, None) for name, df in self.raw_contents.items()}

        self.header = self._get_header()
        self.data = self._get_data(self.header.size + 1) # plus 1 for empty line between header and data
