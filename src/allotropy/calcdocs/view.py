...
class Keys:
    ...
    def head_tail(self) -> tuple[Keys, Keys]:
        head_key, *tail_keys = self.entries
        return Keys(entries=(head_key,)), Keys(entries=tuple(tail_keys))
...
class ViewData:
    ...
    def get_item(self, keys: Keys) -> ViewData | list[Element]:
        ...
        head, tail = keys.head_tail()
        item = self.data[head.get(self.name).value]
