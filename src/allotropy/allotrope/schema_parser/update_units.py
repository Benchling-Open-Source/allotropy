...
def _get_quantity_value_schema(unit_name: str, prefix: Optional[str] = "") -> dict[str, Any]:
    return {
        "allOf": [
            {
...
def update_unit_files(missing_unit_to_iri: dict[str, str]) -> None:
    ...
    custom_schema = {}
    for unit_name in units_schema:
        custom_schema[f"tQuantityValue{unit_name}"] = _get_quantity_value_schema(unit_name)
        custom_schema[f"tNullableQuantityValue{unit_name}"] = _get_quantity_value_schema(unit_name, "Nullable")
