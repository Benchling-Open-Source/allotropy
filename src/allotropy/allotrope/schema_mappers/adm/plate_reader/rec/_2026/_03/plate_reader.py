...
from allotropy.allotrope.models.adm.plate_reader.rec._2026._03.plate_reader import (
    ...
    DeviceSystemDocument as PlateReaderDeviceSystemDocument,
    FluorescenceEmissionProfileDataCube,
    FluorescenceEmissionSpectrumDataCube,
...
class Measurement:
    ...
    illumination_mode_setting: str | None = None
...
class Mapper(SchemaMapper[Data, Model]):
    MANIFEST = "http://purl.allotrope.org/manifests/plate-reader/REC/2026/03/plate-reader.manifest"

    def map_model(self, data: Data) -> Model:
        return Model(
            plate_reader_aggregate_document=PlateReaderAggregateDocument(
                device_system_document=PlateReaderDeviceSystemDocument(
    ...
    def _get_processed_data_aggregate_document(
        self, processed_data: ProcessedDataDocument | None
    ) -> ProcessedDataAggregateDocument | None:
        if not processed_data:
            return None

        processed_data_item = ProcessedDataDocumentItem(
            processed_data_identifier=processed_data.identifier,
            peak_list=PeakList(
                peak=[
                    add_custom_information_document(
                        PeakItem(),
                    ...
                    for peak_custom_info in processed_data.peak_list_custom_info
                ]
            )
            if processed_data.peak_list_custom_info
            else None,
        )

        if processed_data.concentration_factor is not None:
            processed_data_item = add_custom_information_document(
                processed_data_item,
                {
                    "concentration factor": quantity_or_none(
                        TQuantityValueNanogramPerMicroliter,
                        processed_data.concentration_factor,
                    )
                },
            )

        return ProcessedDataAggregateDocument(
            processed_data_document=[processed_data_item]
        )
