    ...
    try:
        ...
        for schema, keywords in indicators.items():
            ...
            if matches > 0:
                ...
                analysis["measurement_indicators"].extend(
                    [kw for kw in keywords if kw in content_str]
                )
...
def main():
    ...
    if analysis.get("suggested_schema"):
        ...
        print("\nNext steps:")
        print(
            f"1. Run 'python .claude/skills/parser-generator/scripts/list_schemas.py {analysis['suggested_schema']}' for more details"
        )
        print(
            "2. Review existing parsers in src/allotropy/parsers/ for similar instruments"
        )
        print("3. Run 'python scripts/create_parser.py' to generate the parser")
    else:
